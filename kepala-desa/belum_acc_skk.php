								...
								<tbody>
									...
									?>
										<tr>
											...
											<td><img src="../style/img/scan_kk_k/<?php echo $kk; ?>" width="50" height="50" alt=""></td>
											<td>
												<!-- Tombol Download File -->
												<a href="../staf/downloads/Surat_Keterangan_Kematian_<?php echo $nama_almarhum; ?>_<?php echo $id; ?>.pdf" class="btn btn-link btn-primary btn-lg" download>
													<i class="fa fa-download"></i> Download
												</a>
												<!-- Form Upload File yang sudah ditandatangani -->
												<?php if (empty($scan_skk)) { ?>
													<input type="file" name="signed_file" required>
													<input type="hidden" name="id_request_skk" value="<?php echo $id; ?>">
...
if (isset($_POST['upload'])) {
	$id_request_skk = $_POST['id_request_skk'];
	...
	if (move_uploaded_file($_FILES["signed_file"]["tmp_name"], $target_file)) {
		...
		// Update kolom scan_skk di database dan setujui permohonan
		$sql = "UPDATE data_request_skk SET scan_skk = ?, acc = ?, status = 2, keterangan = 'Surat sedang dalam proses cetak' WHERE id_request_skk = ?";
		$stmt = mysqli_prepare($konek, $sql);
		$tgl_acc = date('Y-m-d'); // Tanggal saat ini
		mysqli_stmt_bind_param($stmt, 'ssi', $filename, $tgl_acc, $id_request_skk);
